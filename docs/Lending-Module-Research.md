...
The pool no longer fixes its price oracle at construction — it resolves `LLM_USDC_PRICE_ORACLE_POLICY` live from the kernel. At launch there is no liquid LLM/USDC pair, so the oracle is a manual `FixedLlmUsdcPriceOraclePolicy` whose price is changed by a module-governance repoint (referendum is the deliberate, review-gated update path — no trusted price key). **Launch price (owner decision, 2026-07): 1 LLM = 2 USDC**, i.e., the launch oracle is constructed with `assetUnitsPerLlm = 2_000_000` (2 USDC in 6-decimal units); this takes effect when the lending module is added to the deploy scripts (queued — lending is currently test-only, deployed by neither production nor demo scripts). When the Uniswap V4 LLM/USDC pair has enough liquidity, governance repoints the module to a V4 TWAP oracle without redeploying the pool. This implements the launch phase of open question #1.

### Bad-debt backstop

`USDCLendingPoolApp.absorbBadDebt(personId)` writes off a position's residual debt once liquidators have seized all seizable surplus (only the untouchable 5,000-LLM citizenship floor remains, so the debt is structurally unrecoverable — a real possibility here because that floor can never be seized). Protocol reserves are first-loss capital and absorb the write-off before suppliers; any remainder lowers LP share value until governance restores it with a referendum-approved treasury disbursement to the pool (a plain USDC transfer in raises cash and share value). This is the treasury-absorption backstop (Aave's Jan 2023 choice) rather than silent supplier socialization, and closes the backstop half of open question #2.
...
2. **Liquidation economics for illiquid seized collateral. (Decided + backstop shipped 2026-07.)** Owner decision: **accept it** — only long-term LLM accumulators will liquidate, which is aligned with citizenship — with the 10% bonus **votable** (it is a governed parameter in `LendingRiskParameterPolicy`, retunable by referendum). The **bad-debt backstop shipped**: `absorbBadDebt` writes off unrecoverable residual debt, reserves-first then treasury coverage (§6). A Dutch-auction liquidation remains an optional future refinement if fixed-bonus liquidations stall.
